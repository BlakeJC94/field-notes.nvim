...
local function set_update_index_command()
    for _, cmd in ipairs({"FieldNotesIndexUpdate"}) do
        vim.api.nvim_create_user_command(
            cmd,
            function(keys)
                require("field_notes.core.note").update_index(keys)
            end,
            { force = true, }
        )
    end
